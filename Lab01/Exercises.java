...
*/
   public static int squares(int n)
   {
      ...
      if(n < 0){
         ...
         // Loop through until n and add on index ^ 2
         int num = 0;
         for(int i = 1; i < n + 1; i++){
            num = num + (i * i);
         }
...
public static int maxIndex(int array[],int len)
{
      ...
      // Else loop through the array and find the max
      else{
         int max = 0;
         int maxIndex = 0;
         for(int i = 0; i < len; i++){
...
public static boolean seek(int array[], int len, int target){
   ...
   // Loop through the array and look for target
   // If so convert flag to true
   for(int i = 0; i < len; i++){
      if(array[i] == target){
         hit = true;
      }
...
{
   // Check if n = 1 return 0 if so
   if(n < 1){
      return 0;
   }
   
   // Create an array for the fib nums
   int fibArr[] = new int[n+2];
   
   // Give every element the value of -1
   for(int i = 0; i < fibArr.length -1; i++){
      fibArr[i] = -1;
   }
   
   // Assign the starting values for the first two values in the sequence
   fibArr[0] = 0;
   fibArr[1] = 1;
   
   // Loop through the array starting at two calculating the sequence
   for(int i = 2; i <= n; i++){
